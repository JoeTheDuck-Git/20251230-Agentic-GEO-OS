...
import { assetIdFromSpec } from '@/lib/geo/measurement/mapToAsset';
import { GeoDataFreshness } from '@/components/geo/GeoDataFreshness';
import { getContentSpecsMeta } from '@/lib/geo/meta/geoMeta';
...
function ContentSpecsContent() {
  ...
  const freshnessMeta = getContentSpecsMeta(geoQueryState);
  ...
  useEffect(() => {
    ...
    // Priority: spec > fromBrief > fromClaim > default
    if (specId) {
    ...
    if (fromClaim) {
      // Claim-based spec creation is not yet implemented
      // Show stub state - user can see available specs but no auto-selection
      setSelectedSpecId(null);
      return;
    }
  ...
  // Show stub notice for fromClaim
  const showClaimStub = fromClaim !== null;
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {/* Claim-based spec creation stub notice */}
      {showClaimStub && (
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">
                Claim-based Content Spec creation
              </p>
              <p className="text-xs text-yellow-700 mt-1">
                Content Spec creation from Market Voice claims (claim ID: {fromClaim}) is not yet implemented. 
                Please select an existing spec from the list below or create a new spec manually.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="rounded-lg border bg-card p-6 space-y-6">
              ...
              <div>
                ...
                <div className="mt-4 flex items-center gap-4">
                  ...
                  {(() => {
                    const mappedAssetId = assetIdFromSpec(selectedSpec.id);
                    if (mappedAssetId) {
                      return (
                        <GeoLink
...
`;
            ...
            <GeoEmptyState
              title={EMPTY_STATE_COPY.SPEC_NOT_FOUND.TITLE}
              description={EMPTY_STATE_COPY.SPEC_NOT_FOUND.DESC}
